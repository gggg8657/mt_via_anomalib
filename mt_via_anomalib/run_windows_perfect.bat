@echo off
chcp 65001 > nul
echo 🏆 완벽한 최종 버전 - Windows용 커스텀 비디오 데이터셋 AI-VAD 학습 (GPU 지원)
echo ==========================================================
...
if exist "custom_results_perfect" (
    rmdir /s /q "custom_results_perfect" 2>nul
    echo ✅ custom_results_perfect 폴더 정리 완료
)
if exist "custom_avenue_perfect" (
    rmdir /s /q "custom_avenue_perfect" 2>nul
    echo ✅ custom_avenue_perfect 폴더 정리 완료
)
...
REM 학습 실행
echo.
echo 🎯 완벽한 최종 버전 AI-VAD 모델 학습을 시작합니다...
...
if %ERRORLEVEL% NEQ 0 (
    ...
    echo.
    echo 💡 해결 방법:
    echo 1. train_custom_perfect.py 파일에서 video_files 리스트에 비디오 파일 경로를 추가하세요
    ...
    echo 6. GPU 사용을 위해서는 gpu_diagnostic.py를 실행하여 GPU 상태를 확인하세요
    echo.
...
echo.
echo 📁 생성된 파일:
echo - aivad_custom_perfect_checkpoint.ckpt (학습된 모델)
echo.
echo 🚀 다음 단계:
echo 1. realtime_ui_advanced_windows.py 실행
echo 2. "체크포인트 로드" 버튼 클릭
echo 3. aivad_custom_perfect_checkpoint.ckpt 파일 선택
echo 4. 웹캠이나 영상 파일로 실시간 이상 탐지 테스트
echo.
echo 📖 사용법은 WINDOWS_SETUP_GUIDE.md 파일을 참조하세요.
echo.
echo 💡 완벽한 최종 버전 특징:
echo - 모든 알려진 오류 완전 해결 (Windows 권한, CustomVideoDataModule, pandas DataFrame, 변수 스코프, 경로 처리, IndexError)
echo - 비디오 프레임 수 기반 ground truth 생성
