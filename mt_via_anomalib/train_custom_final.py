    ...
    for i, video_file in enumerate(video_files):
        ...
        try:
            ...
            if i < len(video_files) // 2:  # 절반만 테스트용으로
                ...
                gt_dest = gt_path / f"{i+1}_label" / f"{j:04d}.png"
                gt_dest.parent.mkdir(exist_ok=True)
                gt_dest.touch()  # 빈 파일 생성
...
""")
    ...
    try:
        datamodule = Avenue(
            root=dataset_path,
            gt_dir=dataset_path / "ground_truth_demo",  # ground truth 경로 명시
