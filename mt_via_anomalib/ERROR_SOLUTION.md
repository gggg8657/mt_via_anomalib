...
**`train_custom_final.py`** - 최종 안정적인 버전 ⭐
...
1. **첫 번째 시도**: `train_custom_final.py` (최고 안정적) ⭐
2. **두 번째 시도**: `train_custom_fixed.py` (안정적)
3. **세 번째 시도**: `train_aivad_simple.py` (Avenue 데이터셋으로 테스트)
4. **네 번째 시도**: `train_custom_windows_fix.py` (Windows 특화)
