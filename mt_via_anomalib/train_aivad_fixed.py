"""
AI-VAD 올바른 학습 방법 (최종 수정 버전)
이미지를 비디오 시퀀스로 변환하여 AI-VAD 학습
"""
...
def create_fixed_dataset_from_json(json_path="image_segments.json", target_dir="fixed_dataset"):
    """JSON에서 정상 프레임들을 수정된 데이터셋으로 변환"""
    print(f"📁 AI-VAD용 수정된 데이터셋 생성: {json_path}")
...
def main():
    """메인 함수"""
    print("🚀 AI-VAD 올바른 학습 방법 (최종 수정 버전)")
    print("=" * 60)
    print("💡 핵심 원리:")
    ...
    # 1. 수정된 데이터셋 생성
    dataset_root = create_fixed_dataset_from_json()
    if dataset_root is None:
        print("❌ 수정된 데이터셋 생성 실패")
        return
    ...
    # 6. AI-VAD 학습 (비디오 시퀀스로)
    print(f"\n🎯 AI-VAD 학습 시작 (비디오 시퀀스)...")
    print("💡 학습 과정:")
    ...
    try:
        # AI-VAD의 올바른 학습 방법 (비디오 시퀀스)
        # PyTorch Lightning 대신 직접 학습
        ...
        for batch_idx, batch in enumerate(dataloader):
            try:
                ...
                # AI-VAD 모델 호출 (Feature 추출)
                with torch.no_grad():
                    features_per_batch = model.model(video_tensor)
                
                # Density estimator 업데이트
    ...
    # 7. 체크포인트 저장
    checkpoint_path = "aivad_fixed_learned.ckpt"
    try:
        # AI-VAD 모델 상태 저장
        torch.save({
            ...
            'model_class': 'AiVad',
            'training_type': 'fixed_density_estimation',
    ...
    print("💡 학습된 내용:")
    print("1. 이미지 → 비디오 시퀀스 변환")
    print("2. 정상 데이터의 Feature 분포 학습")
    print("3. Density Estimator로 이상 탐지 준비")
    print("4. UI에서 'aivad_fixed_learned.ckpt' 로드하여 테스트")
    print("5. 비정상 데이터는 분포에서 벗어나 높은 점수")
